...
def wsandles(request,name=None):

    if name:
        products=Product.objects.filter(brand__name=name,gender__name='Female')
    else:
        products = Product.objects.filter(cat__name='Sandles',gender__name='Female')
    ...
    return render(request, 'wsandles.html', {
        'username': user_nm,
        "data": product_list,
        "time": msg,
        "fst": fst,
